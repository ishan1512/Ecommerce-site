...
export const getFeaturedProducts=async(req,res)=>{
    try {
        ...
        //.lean() will return a plain javascript objectintead of mongodb document whic is good for performance
...
export const toggleFeaturedProduct=async(req,res)=>{
    try {
        ...
        console.log("Error in toggleFeatyredProdut controller", error.message)
        res.status(500).json({message:"Server error", error:error.message});
    }
}
...
export const deleteProduct=async(req,res)=>{
    try {
        ...
        if(product.image){
            const publicId = product.image.split("/").pop().split(".")[0];//this will get the image id from cloudinary
            try {
